...
import GrafoVisualization from '../components/GrafoVisualization'
...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ noticias: null, fuentes: null, usuarios: null, claims: null })
  const [statsLoading, setStatsLoading] = useState(true)

  useEffect(() => {
    Promise.all([
      fetch('/api/v1/noticias?page=0&size=1').then(r => r.json()),
      fetch('/api/v1/fuentes').then(r => r.json()),
      fetch('/api/v1/usuarios').then(r => r.json()),
      fetch('/api/v1/claims').then(r => r.json()),
    ])
      .then(([n, f, u, c]) => {
        setStats({ noticias: n.totalElements ?? 0, fuentes: f.length, usuarios: u.length, claims: c.length })
  ...
  const grafoNodes = noticias.flatMap(n => {
    const nodes = [{ id: n.id, type: 'noticia', titulo: n.titulo, scoreCredibilidad: n.scoreCredibilidad }]
    if (n.fuente) nodes.push({ id: `fuente-${n.fuente.id}`, type: 'fuente', nombre: n.fuente.nombre })
    if (n.tema) nodes.push({ id: `tema-${n.tema.id}`, type: 'tema', nombre: n.tema.nombre })
    return nodes
  })

  const grafoLinks = noticias.flatMap(n => {
    const links = []
    if (n.fuente) links.push({ source: n.id, target: `fuente-${n.fuente.id}` })
    if (n.tema) links.push({ source: n.id, target: `tema-${n.tema.id}` })
    return links
  })
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatsCard icon={<ExclamationTriangleIcon className="w-6 h-6" />} label="Claims" value={stats.claims} color="yellow" loading={statsLoading} />
      </div>
