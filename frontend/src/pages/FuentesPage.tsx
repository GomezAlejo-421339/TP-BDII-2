...
interface Fuente {
  ...
  dominio: string
  verificada: boolean
  puntajeHistorial: number | string
}

const empty: Fuente = { id: '', nombre: '', dominio: '', verificada: false, puntajeHistorial: '' }

export default function FuentesPage() {
  const [fuentes, setFuentes] = useState<Fuente[]>([])
  const [loading, setLoading] = useState(true)
  const [form, setForm] = useState<Fuente | null>(null)
  ...
  const openEdit = (f: Fuente) => setForm({ ...f, puntajeHistorial: f.puntajeHistorial ?? '' })
  ...
  const handleDelete = async (f: Fuente) => {
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {fuentes.map(f => {
          const puntaje = typeof f.puntajeHistorial === 'string' ? parseFloat(f.puntajeHistorial) : f.puntajeHistorial
          return (
            <div key={f.id} className="bg-white border border-gray-200 rounded-xl p-4">
              ...
              {puntaje != null && !isNaN(puntaje) && (
                <div className="mt-2 flex items-center gap-2">
                  <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                    <div
                      className={`h-1.5 rounded-full ${puntaje > 0.6 ? 'bg-green-500' : 'bg-yellow-500'}`}
                      style={{ width: `${puntaje * 100}%` }}
                    />
                  </div>
                  <span className="text-xs text-gray-500">{Math.round(puntaje * 100)}%</span>
                </div>
              )}
          ...
          <form onSubmit={handleSave} className="bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={String(form.verificada)} onChange={e => setForm({ ...form, verificada: e.target.value === 'true' })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
