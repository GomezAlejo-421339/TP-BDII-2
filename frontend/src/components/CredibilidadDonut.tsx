...
const COLORS: Record<string, string> = { 'Confiable': '#22c55e', 'Dudosa': '#eab308', 'Crítica': '#ef4444' }

export interface CredibilidadDonutItem {
  rango: string
  cantidad: number
  porcentaje: number
}

export interface CredibilidadDonutProps {
  data?: CredibilidadDonutItem[]
  loading?: boolean
}

type PieDatum = d3.PieArcDatum<CredibilidadDonutItem>

export default function CredibilidadDonut({ data, loading }: CredibilidadDonutProps) {
  ...
  useEffect(() => {
    ...
    const pie = d3.pie<CredibilidadDonutItem>()
      .value(d => d.cantidad)
      .sort(null)

    const arc = d3.arc<PieDatum>()
      .innerRadius(radius * 0.5)
      .outerRadius(radius)

    const arcHover = d3.arc<PieDatum>()
      .innerRadius(radius * 0.5)
      .outerRadius(radius + 10)

    const paths = g.selectAll<SVGPathElement, PieDatum>('path')
      .data(pie(sorted))
      .enter()
      .append('path')
      .attr('d', arc)
    ...
    const labels = g.selectAll<SVGTextElement, PieDatum>('.arc-label')
      .data(pie(sorted))
      .enter()
      .append('text')
