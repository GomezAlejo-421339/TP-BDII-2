...
export interface FuenteBarChartItem {
  fuente: string
  total: number
  verificada: boolean
...
export interface FuentesBarChartProps {
  data?: FuenteBarChartItem[]
  loading?: boolean
}

export default function FuentesBarChart({ data, loading }: FuentesBarChartProps) {
  ...
  useEffect(() => {
    ...
    const rects = svg.selectAll<SVGRectElement, FuenteBarChartItem>('rect')
      .data(data)
      .enter()
      .append('rect')
      .attr('x', d => x(d.fuente) ?? 0)
