...
export interface TendenciaLineChartItem {
  fecha: string
  total: number
  credibilidadPromedio: number
}

export interface TendenciaLineChartProps {
  data?: TendenciaLineChartItem[]
  loading?: boolean
}

export default function TendenciaLineChart({ data, loading }: TendenciaLineChartProps) {
  ...
  useEffect(() => {
    ...
    svg.append('g')
      ...
      ).tickFormat(d => d && d.length > 10 ? d.slice(0, 10) : (d || '')))
      .selectAll('text')
      .style('font-size', '9px')
      .style('fill', '#64748b')
    ...
    const lineTotal = d3.line<TendenciaLineChartItem>()
      .x(d => x(d.fecha) ?? 0)
      .y(d => y(d.total))
      .curve(d3.curveMonotoneX)

    const lineCred = d3.line<TendenciaLineChartItem>()
      .x(d => x(d.fecha) ?? 0)
