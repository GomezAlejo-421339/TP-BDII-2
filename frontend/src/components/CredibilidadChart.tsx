...
export interface CredibilidadChartProps {
  noticias: Noticia[]
}

export default function CredibilidadChart({ noticias }: CredibilidadChartProps) {
  ...
  useEffect(() => {
    ...
    const bins = d3.bin<number, number>().domain([0, 1]).thresholds(10)(scores)
    ...
    svg.selectAll('.bar')
      ...
      .attr('class', 'bar')
      .attr('x', d => x(d.x0 ?? 0) + 2)
      .attr('width', d => Math.max(0, x(d.x1 ?? 0) - x(d.x0 ?? 0) - 3))
      ...
      .on('mouseenter', function (event: MouseEvent, d) {
        ...
        tooltip.style('display', 'block')
          ...
          .html(`<strong>${(d.x0 ?? 0).toFixed(1)} - ${(d.x1 ?? 0).toFixed(1)}</strong><br/>${d.length} noticias`)
      })
